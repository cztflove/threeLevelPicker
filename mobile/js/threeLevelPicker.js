...
;(function(root,factory){
	...
	threeLevelPicker.prototype = {
		...
		createEntity: function(){
			 ...
			 var dom = '<div class="threePicker-layout picker-hide" id="'+ pickerId +'"><div class="ub ub-ver picker-content">';
			 dom += '<div class="ub ub-ac picker-tab">';
		...
		bindEvent: function(){
			...
			_this.$pickerEle.find('.picker-list').on('click','li',function(){
				...
				if(parentId == 'picker-list'+ _this.idNum +'-1'){
					...
					if(_this.opt.levelName.length == 3){
						...
						_this.$pickerEle.removeClass('on').on('transitionend webkitTransitionEnd',function(){
			...
			_this.$pickerEle.find('.picker-close-btn').on('click',function(){
				_this.$pickerEle.removeClass('on').on('transitionend webkitTransitionEnd',function(){
					_this.$pickerEle.addClass('picker-hide');
					$(this).off('transitionend webkitTransitionEnd');
				});
			...
			$( _this.opt.trigger).click(function(event) {
				_this.$pickerEle.removeClass('picker-hide').addClass('on');
			});
		}
	};
